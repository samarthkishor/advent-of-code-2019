...
let read_lines filename =
  let open Stdio in
  let line = In_channel.create filename |> In_channel.input_line in
  match line with None -> "" | Some l -> l
;;

let array_of_line line =
  String.split line ~on:',' |> List.map ~f:Int.of_string |> Array.of_list
;;
...
let to_opcode n = match n with 99 -> Halt | 1 -> Add | 2 -> Multiply | _ -> Num n
let opcode_array array = Array.map array ~f:to_opcode
let int_of_opcode op = match op with Num n -> n | Add -> 1 | Multiply -> 2 | Halt -> 99

let rec execute_opcodes i array =
  match array.(i) with
  | Halt -> array
  | Add ->
    array.(array.(i + 3) |> int_of_opcode)
    <- to_opcode
         ( int_of_opcode array.(array.(i + 1) |> int_of_opcode)
         + int_of_opcode array.(array.(i + 2) |> int_of_opcode) );
    execute_opcodes (i + 4) array
  | Multiply ->
    array.(array.(i + 3) |> int_of_opcode)
    <- to_opcode
         ( int_of_opcode array.(array.(i + 1) |> int_of_opcode)
         * int_of_opcode array.(array.(i + 2) |> int_of_opcode) );
    execute_opcodes (i + 4) array
  | Num _ -> execute_opcodes (i + 1) array
;;

let string_of_opcode_array array =
  Array.map array ~f:int_of_opcode
  |> Array.map ~f:(Printf.sprintf "%d")
  |> Array.to_list
  |> String.concat ~sep:","
;;

let part1 () =
  let array = read_lines "./day2_input.txt" |> array_of_line |> opcode_array in
  array.(1) <- to_opcode 12;
  array.(2) <- to_opcode 2;
  let result = execute_opcodes 0 array in
  Stdio.printf "%d\n" (int_of_opcode result.(0))
;;
...
let part2 () =
  let program = read_lines "./day2_input.txt" |> array_of_line |> opcode_array in
  for noun = 0 to 99 do
    for verb = 0 to 99 do
      let new_program = Array.copy program in
      new_program.(1) <- to_opcode noun;
      new_program.(2) <- to_opcode verb;
      let run_program = execute_opcodes 0 new_program in
