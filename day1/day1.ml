 ...
 *     subtract 2 to get 2.  For a mass of 14, dividing by 3 and rounding
 *     down still yields 4, so the fuel required is also 2.  For a mass
 *     of 1969, the fuel required is 654.  For a mass of 100756, the fuel
 *     required is 33583.
 *
...
let total_fuel lines =
  let fuel_list = List.map ~f:Int.of_string lines |> List.map ~f:calculate_fuel in
  List.fold fuel_list ~init:0 ~f:( + )
;;

let part1 () = read_lines "./day1_input.txt" |> total_fuel |> Stdio.printf "%d\n"
 ...
 *     negative fuel), so the total fuel required is still just 2.  At
 *     first, a module of mass 1969 requires 654 fuel. Then, this fuel
 ...
 *     of mass 1969 is 654 + 216 + 70 + 21 + 5 = 966.  The fuel required
 *     by a module of mass 100756 and its fuel is: 33583 + 11192 + 3728 +
...
let rec module_fuel mass =
  if mass <= 0
  then 0
  else
    let new_fuel = max 0 (calculate_fuel mass) in
    new_fuel + module_fuel new_fuel
;;

let new_total_fuel lines =
  let fuel_list = List.map ~f:Int.of_string lines |> List.map ~f:module_fuel in
  List.fold fuel_list ~init:0 ~f:( + )
;;

let part2 () = read_lines "./day1_input.txt" |> new_total_fuel |> Stdio.printf "%d\n"
